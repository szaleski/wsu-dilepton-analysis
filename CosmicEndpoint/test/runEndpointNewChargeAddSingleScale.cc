...
void chargeLoopBias(std::string const& inDir1, std::string const& inDir2, TFile *oFile, int nBiasBins, int rebins, std::string const& histBase, TH1F *chi2Hist, std::string const& algo, std::string const& comp, double rawMinusScale, double rawPlusScale);


void runEndpointNewChargeAddSingleScale(std::string const& inDir1, std::string const& inDir2, std::string const& outFile, int nBiasBins, int rebins, std::string const& histBase, std::string const& comp){
  ...
  */
  ...
  double refMinusInt = refMinusHist->Integral();
  double refPlusInt = refPlusHist->Integral();
  double mcMinusInt = mcHistMinus->Integral();
  double mcPlusInt = mcHistPlus->Integral();
  
  double minusScale = refMinusInt/mcMinusInt;
  double plusScale = refPlusInt/mcPlusInt;


    TH1F *chi2Hist = new TH1F(TString("Chi2_"+algo), TString("Chi2_"+algo),100 , 0., 150);
    chargeLoopBias(inDir1, inDir2, oFile2, nBiasBins, rebins, histBase, chi2Hist, algo, comp, minusScale, plusScale);//, dir);
    //  }
...
void chargeLoopBias(std::string const& inDir1, std::string const& inDir2, TFile *oFile, int nBiasBins, int rebins, std::string const& histBase, TH1F *chi2Hist, std::string const& algo, std::string const& comp, double rawMinusScale, double rawPlusScale){//, TDirectory dir){
  ...
  if(g == 0){
	...
	mcHistPlus->Scale(rawPlusScale);
	mcHistMinus->Scale(rawMinusScale);
    ...
    if(algo == "TuneP"){
      ...
      can->SaveAs(TString("~/public/html/cosmics/"+comp+"Qsep_Added_Chi2rebin"+numRebin.str()+"SS.png"));
    }
