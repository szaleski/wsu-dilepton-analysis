...
if __name__ == "__main__":
    ...
    print histName
    for step in range(0,1000/options.stepsize):
        gifcanvas.cd()
        negBias = myInFile.Get("%s%sMinusBias%03d"%(histName,"MinusCurve",1000-step*options.stepsize))
        negBias = setMinPT(negBias,options.totalbins,200./1000.,False)
        negBias.Rebin(options.rebins)
        posBias = myInFile.Get("%s%sMinusBias%03d"%(histName,"PlusCurve", 1000-step*options.stepsize))
        posBias = setMinPT(posBias,options.totalbins,200./1000.,False)
        ...
        negBias.SetTitle("#Delta#kappa = %2.4f [c/TeV]"%(-(options.maxbias/1000.)*(1000-step*options.stepsize)))
        ...
        thelegend.Draw()
        r.gPad.Update()
        gifcanvas.SaveAs("%s/biasBin%04d_asym.png"%(gifDir,step*options.stepsize))
    ...
    #negBias.FindObject("stats").SetOptStat(0)
    #posBias.FindObject("stats").SetOptStat(0)
    gifcanvas.SaveAs("%s/biasBin%04d_asym.png"%(gifDir,1000))
    r.gPad.Update()
    chi2Val  = r.Double(0.) # necessary for pass-by-reference in python
    chi2ndf  = r.Long(0)    # necessary for pass-by-reference in python
    igood    = r.Long(0)    # necessary for pass-by-reference in python
    ...
    for step in range(0,1000/options.stepsize):
        ...
        neginta = negBias.Integral()
        posinta = posBias.Integral()
        negBias.SetTitle("#Delta#kappa = %2.4f [c/TeV]"%((options.maxbias/1000.)*(step*options.stepsize+1)))
        negBias.SetLineColor(r.kRed)
        negBias.SetLineWidth(2)
        negBias.Draw()
        negBias.GetXaxis().SetTitle("#kappa [c/TeV]")
        ...
        thelegend.Draw()
        r.gPad.Update()
        gifcanvas.SaveAs("%s/biasBin%04d_asym.png"%(gifDir,1000+1+step*options.stepsize))
