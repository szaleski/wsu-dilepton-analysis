...
void makeThePlotsQaddedSS(std::string const& inFile1, std::string const& inFile2, std::string const& oDir, int numBin){
  ...
  if(numBin > 0){
    ...
    TH1F* dataHistMinusScaled = (TH1F*)f->Get("TunePCraftMinus;2");
    std::cout << "\nGot the first histo\n";
    std::cout << "\nfirst histo:" << std::hex << dataHistMinusScaled << std::dec << std::endl;  
    TH1F* dataHistPlusScaled = (TH1F*)f->Get("TunePCraftPlus;2");
    std::cout << "\nGot the first histo\n";
    std::cout << "\nfirst histo:" << std::hex << dataHistPlusScaled << std::dec << std::endl;  


    TH1F* dataHistCut = (TH1F*)f->Get("TunePCraftPlus_AfterCut");
    std::cout << "\nGot the second histo\n";
    std::cout << "\nSecond histo:" << std::hex << dataHistCut << std::dec << std::endl;  

    TH1F* dataHistRebin = (TH1F*)f->Get("TunePCraftPlus_Rebinned");
    std::cout << "\nGot the third histo\n";
    std::cout << "\nThird histo:" << std::hex << dataHistRebin << std::dec << std::endl;  

    TH1F* mcHistMinus = (TH1F*)f->Get(TString("TunePMC_Curve_Minus_PlusBias"+ss.str()));
    std::cout << "\nGot the added MC histo!";
    std::cout << "\nMC histo:" << std::hex << mcHistMinus << std::dec << std::endl;
    TH1F* mcHistPlus = (TH1F*)f->Get(TString("TunePMC_Curve_Plus_PlusBias"+ss.str()));
    std::cout << "\nGot the added MC histo!";
    std::cout << "\nMC histo:" << std::hex << mcHistPlus << std::dec << std::endl;

    TH1F* mcHistMinusScaled = (TH1F*)f->Get(TString("TunePMCMinusCurvature_Scaled_PlusBias"+ss.str()));
    std::cout << "\nGot the fourth histo\n";
    std::cout << "\nfourth histo:" << std::hex << mcHistMinusScaled << std::dec << std::endl;  
    TH1F* mcHistPlusScaled = (TH1F*)f->Get(TString("TunePMCPlusCurvature_Scaled_PlusBias"+ss.str()));
    std::cout << "\nGot the fourth histo\n";
    std::cout << "\nfourth histo:" << std::hex << mcHistPlusScaled << std::dec << std::endl;  

    TH1F* mcHistCut = (TH1F*)f->Get(TString("TunePMCPlus_Curve_AfterCut_PlusBias"+ss.str()));
    std::cout << "\nGot the fifth histo\n";
    std::cout << "\nfifth histo:" << std::hex << mcHistCut << std::dec << std::endl;  
    TH1F* mcHistRebin = (TH1F*)f->Get(TString("TunePMCPlus_Curve_Rebinned_PlusBias"+ss.str()));
    std::cout << "\nGot the last histo\n";
    std::cout << "\nlast histo:" << std::hex << mcHistRebin << std::dec << std::endl;  
  
    TH1F* chi2Hist = (TH1F*)f->Get(TString("TunePChi2vCurvePlus"));
    std::cout << "\nGot the Chi2 histo\n";
    std::cout << "\nchi2 histo:" << std::hex << chi2Hist << std::dec << std::endl;
    
    TH1F* ndofHist = (TH1F*)f->Get(TString("TunePNdofPlus_Plot;2"));
    std::cout << "\nGot the Ndof histo\n";
    std::cout << "\nNdof histo:" << std::hex << ndofHist << std::dec << std::endl;

    
    binWidth = dataHistMinusScaled->GetBinWidth(3);
    
    chi2 = chi2Hist->GetBinContent(1000+numBin);
    ndof = ndofHist->GetBinContent(1000+numBin);
    
    double intDataMinus = dataHistMinusScaled->Integral(251, 1250);
    double intDataPlus = dataHistPlusScaled->Integral(251, 1250);
    double intMcMinusBefore = mcHistMinus->Integral(251, 1250);
    double intMcPlusBefore = mcHistPlus->Integral(251, 1250);
    double intMcMinusAfter = mcHistMinusScaled->Integral(251, 1250);
    double intMcPlusAfter = mcHistPlusScaled->Integral(251, 1250);

    double intData = intDataMinus + intDataPlus;
    double intBefore = intMcMinusBefore + intMcPlusBefore;
    double intAfter = intMcMinusAfter + intMcPlusAfter;
    

    dataHistMinusScaled->Sumw2();
    dataHistPlusScaled->Sumw2();
    mcHistMinusScaled->Sumw2();
    mcHistPlusScaled->Sumw2();
    dataHistMinusScaled->Add(dataHistPlusScaled);
    mcHistMinusScaled->Add(mcHistPlusScaled);


    plotIt(mcHistMinusScaled, dataHistMinusScaled, "Scaled", "ScalePlots", binWidth, stepSize, numBin, chi2, ndof, intBefore, intAfter, intData);
    ...
    TH1F* mcHistMinus = (TH1F*)f->Get(TString("TunePMC_Curve_Minus_NoBias"));
    std::cout << "\nGot the added MC histo!";
    std::cout << "\nMC histo:" << std::hex << mcHistMinus << std::dec << std::endl;
    TH1F* mcHistPlus = (TH1F*)f->Get(TString("TunePMC_Curve_Plus_NoBias"));
    std::cout << "\nGot the added MC histo!";
    std::cout << "\nMC histo:" << std::hex << mcHistPlus << std::dec << std::endl;

    TH1F* mcHistMinusScaled = (TH1F*)f->Get(TString("TunePMCMinus_Curvature_Scaled_NoBias"));
    std::cout << "\nGot the fourth histo\n";
    std::cout << "\nfourth histo:" << std::hex << mcHistMinusScaled << std::dec << std::endl;  
    TH1F* mcHistPlusScaled = (TH1F*)f->Get(TString("TunePMCPlus_Curvature_Scaled_NoBias"));
    std::cout << "\nGot the fourth histo\n";
    std::cout << "\nfourth histo:" << std::hex << mcHistPlusScaled << std::dec << std::endl;  

    TH1F* mcHistCut = (TH1F*)f->Get(TString("TunePMCPlus_Curve_AfterCut_NoBias"));
    std::cout << "\nGot the fifth histo\n";
    std::cout << "\nfifth histo:" << std::hex << mcHistCut << std::dec << std::endl;  
    TH1F* mcHistRebin = (TH1F*)f->Get(TString("TunePMCPlus_Curve_Rebinned_NoBias"));
    std::cout << "\nGot the last histo\n";
    std::cout << "\nlast histo:" << std::hex << mcHistRebin << std::dec << std::endl;  
    ...
    TH1F* mcHistMinus = (TH1F*)f->Get(TString("TunePMC_Curve_Minus_MinusBias"+ss.str()));
    std::cout << "\nGot the added MC histo!";
    std::cout << "\nMC histo:" << std::hex << mcHistMinus << std::dec << std::endl;
    TH1F* mcHistPlus = (TH1F*)f->Get(TString("TunePMC_Curve_Plus_MinusBias"+ss.str()));
    std::cout << "\nGot the added MC histo!";
    std::cout << "\nMC histo:" << std::hex << mcHistPlus << std::dec << std::endl;

    TH1F* mcHistMinusScaled = (TH1F*)f->Get(TString("TunePMCMinus_Curvature_Scaled_MinusBias"+ss.str()));
    std::cout << "\nGot the fourth histo\n";
    std::cout << "\nfourth histo:" << std::hex << mcHistMinusScaled << std::dec << std::endl;  
    TH1F* mcHistPlusScaled = (TH1F*)f->Get(TString("TunePMCPlus_Curvature_Scaled_MinusBias"+ss.str()));
    std::cout << "\nGot the fourth histo\n";
    std::cout << "\nfourth histo:" << std::hex << mcHistPlusScaled << std::dec << std::endl;  

    TH1F* mcHistCut = (TH1F*)f->Get(TString("TunePMCPlus_Curve_AfterCut_MinusBias"+ss.str()));
    std::cout << "\nGot the fifth histo\n";
    std::cout << "\nfifth histo:" << std::hex << mcHistCut << std::dec << std::endl;  
    TH1F* mcHistRebin = (TH1F*)f->Get(TString("TunePMCPlus_Curve_Rebinned_MinusBias"+ss.str()));
    std::cout << "\nGot the last histo\n";
    std::cout << "\nlast histo:" << std::hex << mcHistRebin << std::dec << std::endl;  
...
void plotIt(TH1F *mcHist, TH1F *dataHist, std::string const& plotName, std::string const& oDir, Double_t binWidth, double stepSize, int numBin, double chi2, int ndof, double intBefore, double intAfter, double intData){
  ...
  */
  ...
  tmpCanvas->SaveAs(TString(oDir+"/curvature_"+plotName+legendText3.str()+"NewQaddedSS.png"));

  tmpCanvas->SaveAs(TString("~/public/html/cosmics/curvature_"+plotName+legendText3.str()+"NewQaddedSS.png"));
