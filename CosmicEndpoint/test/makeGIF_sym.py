...
if __name__ == "__main__":
    ...
    for step in range(0,options.biasbins/options.stepsize):
        ...
        yVals["rChi2"][step] = chi2Val/chi2ndf
    ...
    yVals["rChi2"][options.biasbins/options.stepsize] = chi2Val/chi2ndf
    ...
    for step in range(0,options.biasbins/options.stepsize):
        ...
        yVals["rChi2"][options.biasbins/options.stepsize+1+step] = chi2Val/chi2ndf
    ...
    graphInfo["Chi2"] = {"color":r.kBlue, "marker":r.kFullCircle,
                         "title":"ROOT #chi^{2}",
                         "yaxis":""}
    graphInfo["rChi2"] = {"color":r.kBlue, "marker":r.kFullCircle,
                         "title":"ROOT #chi^{2}/ndof",
                         "yaxis":""}

    newFile = r.TFile("chi2.root","RECREATE")    

    newFile.cd()
    chi2graph = prettifyGraph(r.TGraph(xVals.size,xVals,yVals["Chi2"]),graphInfo["Chi2"])
    rchi2graph = prettifyGraph(r.TGraph(xVals.size,xVals,yVals["rChi2"]),graphInfo["rChi2"])
    ksgraph   = prettifyGraph(r.TGraph(xVals.size,xVals,yVals["KS"])  ,graphInfo["KS"]  )
    gifcanvas.cd()
    chi2graph.Draw("ALP")
    gifcanvas.SetGridx(1)
    gifcanvas.SetGridy(1)
    chi2graph.GetYaxis().SetRangeUser(0,30)
    chi2graph.GetXaxis().SetRangeUser(-0.2,0.2)
    #chi2graph.SetGridx(1)
    #chi2graph.SetGridy(1)
    #chi2graph.SaveAs("chi2_%s.C"%(options.histbase))
    gifcanvas.SaveAs("chi2_%s.C"%(options.histbase))
    ...
    if options.debug:
        ...
        gifcanvas.SetGridy(1)
        rchi2graph.Draw("ALP")
        #rchi2graph.SetGridx(1)
        #rchi2graph.SetGridy(1)
        rchi2graph.GetYaxis().SetRangeUser(0, 3)
        rchi2graph.GetXaxis().SetRangeUser(-0.2, 0.2)
        #rchi2graph.SetGridx(1)
        #rchi2graph.SetGridy(1)
        #rchi2graph.SaveAs("rchi2_%s.C"%(options.histbase))
        gifcanvas.SaveAs("rchi2_%s.C"%(options.histbase))
