...
void runEndpointNew(std::string const& inDir1, std::string const& inDir2, std::string const& outFile, int nBiasBins, int rebins, std::string const& histBase, std::string const& comp){
  ...
  */
  ...
  loopBias(inDir1, inDir2, oFile, nBiasBins, rebins, histBase, chi2Hist, algo, comp);//, scale, 0);//, dir);
  ...
  if(g == 0){
    ...
    */
    ...
    if(algo == "TuneP"){
      ...
      chi2vCurveHist->SetTitle("#chi^{2}vs#Delta#kappa_{bias}");
      can->SetGridx(1);
      can->SetGridy(1);
      can->Update();
      ...
      leg->Draw();
      can->Update();
      can->SaveAs(TString("~/public/html/cosmics/"+comp+"Chi2rebin"+numRebin.str()+"NewStartup.png"));
    }
