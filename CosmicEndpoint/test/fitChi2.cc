...
void fitChi2(std::string const& rootFile, std::string const& outFile, std::string const& dataHistName){//, double minFit, double maxFit){
  ...
  TH1F* dataHist = (TH1F*)file->Get(TString(dataHistName)); //Use for DatavMC
  ...
  */
  ...
  fitCan->Write();
  fitCan->SaveAs(TString("~/public/html/cosmics/April16/"+outFile+"_fitRangeOnly.png"));
