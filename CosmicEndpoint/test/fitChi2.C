{
  TFile *file = new TFile("/afs/cern.ch/work/s/szaleski/private/CMSSW_7_6_3_patch2/src/WSUDiLeptons/CosmicEndpoint/test/Data2MC.root", "READ");
  TFile *g = new TFile("chi2Fit.root","RECREATE");
  ...
  g->cd();
    ...
    TH1F* dataHist = (TH1F*)file->Get("TunePCraftBoth");
    std::cout << "\nGot the first histo\n";
 ...
 graph->SetTitle("Data_MC_P2_#chi^{2} vs #Delta#kappa_{bias}");
 ...
 can->Update();
 can->Write();
 can->SaveAs("~/public/html/cosmics/data_MCchi2FitP2.png");
